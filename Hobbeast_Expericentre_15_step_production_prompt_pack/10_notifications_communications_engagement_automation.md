# 10. Notifications, communications és engagement automation
...
A jelenlegi realtime notification és preference alapot egységes notification platformmá emelni úgy, hogy hasznos legyen, de ne váljon spamgé.

### Event taxonomy

Definiálj typed notification eseményeket:
- event_invite;
- favorite/recommended event;
- waitlist_promoted;
- organizer_message;
- event_changed/cancelled;
- post_event_feedback;
- mutual_reconnection;
- circle_invite;
- circle_activity;
- hub_opportunity;
- admin/security események csak megfelelő surface-en.

### Architecture

1. canonical notification service;
2. database persistence;
3. realtime in-app;
4. delivery adapters:
   - in-app kötelező;
   - email/push csak provider/config esetén;
5. dedupe key;
6. rate limit;
7. batching/digest;
8. quiet hours opcionális;
9. preference matrix;
10. retry/dead-letter külső channel esetén.

### UX

- NotificationBell marad gyors;
- grouped notifications;
- mark one/all;
- deep link validálva;
- stale target graceful;
- preference center kategóriákkal, nem 30 random togglegal;
- onboarding defaultok ésszerűek.
...
- post-event prompt;
- mutual reconnection;
- dormant circle gentle nudge;
- hub qualification „van elég ember”;
- organizer reminder.

Anti-spam:
- frequency caps;
- ugyanarra az eseményre duplicate notification ne;
- generated/simulated usereknek production delivery ne menjen;
- admin teszt delivery külön sandbox.

### Acceptance

Minden notificationnak typed payload és valid deep link contractja van. User opt-out érvényesül. Blocked relationship miatt nem generálódik social notification. Realtime reconnect nem duplikál.
