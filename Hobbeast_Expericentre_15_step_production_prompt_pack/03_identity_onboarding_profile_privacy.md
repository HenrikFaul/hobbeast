# 03. Identity, onboarding, profil és privacy
...
Production-minőségű consumer onboarding és profilrendszer kialakítása úgy, hogy a Hobbeast már az első sessionben megkapja a social matchinghez szükséges minimális, önkéntes, privacy-safe adatokat, miközben nem kényszeríti túl hosszú regisztrációra a felhasználót.

### Funkciók

1. Auth audit:
   - email/password, Google OAuth, password reset, redirect behavior;
   - email verification állapot;
   - session refresh/loading race;
   - banned/deleted account behavior;
   - auth error mapping magyar user-facing üzenetekkel.

2. Progressive onboarding:
   - display name;
   - avatar opcionális;
   - város / approximate home area, pontos lakcím nem kötelező;
   - 3-10 érdeklődés kiválasztás a canonical hobby taxonomy-ból;
   - preferred activity modes: one-to-one / small group / larger group, csak social preference, nem érzékeny profilozás;
   - availability windows opcionális;
   - experience level opcionális hobbinként;
   - privacy és notification consent;
   - onboarding később folytatható.

3. Profil:
   - public profile DTO;
   - saját profilban private mezők;
   - public view: display name, avatar, city/area coarse level, shared interests, attended public activities opcionálisan;
   - „member since” csak ha termékileg indokolt;
   - exact address/email/phone soha nem public;
   - profile visibility: discoverable / limited / hidden, világos semantics-szel.

4. Hobby preference model:
   - kedvenc category/subcategory/activity meglévő modelljének megőrzése;
   - normalizált relationship table/repository, ha a jelenlegi profile JSON/array későbbi matchinghez kevés;
   - migration backfill idempotensen;
   - virtual hub refresh kompatibilitás.

5. Trust primitives:
   - user block foundation;
   - report foundation;
   - blocked user ne jelenjen meg ajánlásban/circle candidate-ben;
   - report nem automatikus büntetés, admin moderation queue későbbi promptban.

6. Account deletion:
   - social records cascade/anonymization policy előkészítése;
   - event historical integrity vs privacy;
   - generated/simulated user más policy;
   - export/delete request audit.

### UX

A profil ne CV legyen. A fő CTA: „Mit szeretsz csinálni?” és „Mire lennél nyitott a közeledben?”. Az onboarding végén azonnal legyen érték: releváns esemény, hub vagy „még nincs elég ember, értesítünk” állapot.

### Elfogadás

Új user 2-3 percen belül eljut releváns discoveryig. A meglévő régi profilok migration nélkül sem törnek el; backfill kompatibilis. Minden privacy-beállításhoz integration test és RLS/DTO evidence.
