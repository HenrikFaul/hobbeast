# 06. Event lifecycle és participant experience
...
A már meglévő event discovery, create/edit, RSVP, waitlist, event detail, trip planner és external/native event flow-kat egységes production lifecycle-lá emelni.

### Lifecycle state machine

Definiálj explicit eseményállapotokat:
draft -> published -> full/waitlist-capable -> started -> completed -> cancelled/archive.
A meglévő schema viselkedését inventoryzd; ne vezess be duplikált status mezőt, ha már van megfelelő.

Participant:
invited -> going -> waitlist -> checked_in -> completed/attended -> cancelled/no_show.
A meglévő organizer transitionöket és waitlist trigger semantics-et őrizd meg.

### Kötelező fejlesztések

1. Events oldal:
   - URL-persisted filter/query;
   - native/external source filter;
   - hobby/activity;
   - date;
   - distance;
   - capacity;
   - accessible loading/error/empty;
   - pagination/infinite strategy;
   - no duplicate external/native card.
...
2. Event detail:
   ...
   - trip/hike data;
   - participant privacy;
   - external event esetén outbound CTA, Hobbeast „find company/join social group” layer.
...
3. Join:
   ...
   - auto waitlist;
   - cancellation;
   - waitlist auto-promote notification;
   - event cancelled handling.

4. Create/Edit:
   - draft autosave opcionális;
   - category/subcategory/activity canonical;
   - venue suggestions;
   - map/trip planner;
   - template system;
   - capacity;
   ...
   - visibility;
   - validation;
   - organizer ownership authorization.

5. Completion:
   - organizer/event time alapján completion process;
   - checked-in -> completed;
   - no-show kezelés;
   - post-event social flow trigger a Prompt 04 domainjéhez;
   - ne legyen automatikus „attended” pusztán RSVP-ből.

6. External event social wrapper:
   - külső esemény nem lesz Hobbeast-owned organizer event;
   - külön social attendance intent;
   - source URL integrity;
   - imported event stale/cancelled policy.

### Acceptance

Capacity race-t integration test demonstrálja. Waitlist promóció egyszer történik. Cancelled event nem generál post-event social flow-t. External event source semantics nem keveredik native event organizer jogosultságokkal.
