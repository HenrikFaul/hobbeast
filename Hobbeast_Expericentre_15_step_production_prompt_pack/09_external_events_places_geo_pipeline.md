# 09. External events, places és geo pipeline
...
A meglévő Eventbrite/Ticketmaster/SeatGeek + Geoapify/TomTom/Mapy/local places infrastruktúrát stabil production supply layer-ré tenni. Ez a réteg seed liquidity és helyszínválasztás, nem a termék identitása.
...
- dedup ugyanazon esemény több providerből;
- stale/cancelled update;
- source attribution;
- outbound link;
- import pagination/checkpoint;
- rate limits;
- backoff;
- partial provider outage;
- last-success/last-error admin status;
- idempotens upsert.

### Places

A `place-search` refaktor után:
- local catalog first/controlled fallback;
- address vs venue vs trip_planner funkciócsoport;
- provider config;
- deterministic normalization;
- locale/diacritics;
- proximity;
- category taxonomy;
- phone/web/opening hours optional;
- confidence;
- stale cache strategy.

### Address Manager

A meglévő discovery/task-generator/worker pipeline:
- job state machine;
- lease/lock;
- retry/dead-letter;
- provider quotas;
...
- progress;
- admin resume/cancel;
- geodata DB routing contract.

### Venue suggestion social use

A helyszín ne csak input autocomplete legyen. Social circle/event creation során:
- group centroid csak coarse/privacy-safe módon;
- venue category fit;
- travel distance balance;
- opening hours;
- „mindannyiunkhoz ésszerű” recommendation;
- exact user home coordinate soha ne jelenjen meg.

### Admin

Provider health, sync history, item counts, failure samples redacted, manual re-run, config validation. A nagy `AdminEventbrite` szétbontása characterization test után.

### Acceptance

Egy provider outage nem teszi használhatatlanná az Events oldalt. Sync újrafuttatás nem duplikál. Secrets nem clientben. Stale external event policy dokumentált.
