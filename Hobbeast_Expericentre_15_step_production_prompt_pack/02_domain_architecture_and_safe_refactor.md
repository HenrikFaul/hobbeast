# 02. Domain architektúra és biztonságos refaktor
...
A második lépésben bontsd le a legnagyobb load-bearing modulok kockázatát úgy, hogy a felhasználói viselkedés byte-for-byte/contract-szinten megmaradjon, ahol nincs explicit új product requirement. Ez nem kozmetikai fájldarabolás: világos domain boundary kell a későbbi social platform funkciókhoz.
...
- notification hooks és event/organizer service-ek.

### Kötelező munka

1. Karakterizáció:
   - minden nagy modul előtt teszteld a fontos input/output contractokat;
   - `place-search`: action routing, provider choice, local cache behavior, distance/proximity, fallback, malformed provider result;
   - Events: filter, search, join/waitlist, external/native event megkülönböztetés;
   - AdminEventbrite: provider cards, sync/config/filter;
   - AdminUsers: pagination/filter/bulk selection/hub membership;
   - Organizer: event selection, participant state transitions, check-in, messaging, analytics.

2. Hozz létre domain service réteget:
   - `features/events`
   - `features/organizer`
   - `features/community`
   - `features/admin`
   - `features/places`
   - `features/external-events`
   - `features/notifications`
   A pontos mappanév illeszkedjen a repo stílusához; ne mozgass mindent egy commitban.

3. `place-search`:
   - válaszd szét request validation, provider orchestration, local DB repository, normalization/ranking és response serialization rétegre;
   - a jelenlegi response shape maradjon kompatibilis;
   - provider adapterek külön;
   - abort/timeout;
   - deterministic ranking;
   - reverse lookup jelenlegi „not implemented” viselkedését ne implementáld mellékesen; explicit backlog vagy külön kis safe feature.

4. UI nagy komponensek:
   - container vs presentational;
   - query/mutation logic hook/service-be;
   - form state ne legyen szétszórva;
   - URL/query param legyen shareable, ahol már ma is navigation semantics része;
   - a refaktor közben a visual outputot screenshot/regression smoke-kal tartsd.

5. Típusosság:
   - generált Supabase type a source of truth;
   - `as any` hotspot inventory;
   - új `as any` tilos;
   - régi castot csak akkor válts, ha ugyanabban a körben teszted van;
   - external provider DTO -> normalized domain DTO.

6. Adat-hozzáférés:
   - komponensből direkt `.from()` hívásokat fokozatosan domain repository/service-be;
   - ne csinálj absztrakt „god repository”-t;
   - mutation error contract egységes;
   - optimistic update csak megfelelő rollbackkel.

### Definition of done

A fő nagy fájlok csökkenjenek érdemben, de nem LOC-cél kedvéért. Minden kivont modulnak világos ownershipje legyen. A teljes route viselkedés, event creation/edit, admin import, bulk user, virtual hub admin, organizer, place autocomplete és notification flow maradjon működő. Készíts `docs/DOMAIN_ARCHITECTURE.md`-t és dependency diagramot Mermaidben.
