# 11. AI demand aggregation és automatikus eseménygenerálás
...
A meglévő `auto_event_config` + `generate-hub-events` funkciót valódi demand aggregation engine-né fejleszteni. Az AI nem találhat ki keresletet; csak a rendszerben ténylegesen meglévő, privacy-safe aggregált igényből készíthet eseményjavaslatot.

### Demand qualification

Input:
- virtual hub real member count;
- recent active members;
- explicit interest;
- coarse geo;
- optional availability overlap;
- existing upcoming event overlap;
- event fatigue/cooldown;
- organizer capacity/opportunity.

Generated/simulated userek defaultból kizárva production qualificationből.
...
1. **Proposal**:
   ...
   - suggested area/venue category;
   - target capacity;
   - reason: „12 aktív ember, 8 hétvégi availability”;
   - confidence;
   - provenance.
2. **Publish**:
   - admin vagy community/organizer approval, amíg nincs bizonyított autonóm policy;
   - később explicit auto-publish feature flag.
...
- validation;
- max length;
- no unsafe/illegal activity generation;
- no hallucinated venue availability;
- no private member data a promptba;
- prompt injection ellen az external text sanitization;
- model/provider failure fallback template;
...
- publish history;
- rollback/cancel;
- cost estimate, ha elérhető;
- audit.

### Acceptance

AI soha nem hoz létre eventet qualifying demand nélkül. Model hiba esetén nincs félig létrejött event. Ugyanazon hub/time window nem duplikál. Publish auditált és rollbackolható.
