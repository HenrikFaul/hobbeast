...
A jelenlegi InviteM-ihletésű organizer dashboardot production-képes organizer operating suite-tá fejleszteni, miközben nem fordítod át a teljes consumer terméket organizer SaaS-sá.
...
1. Authorization:
   - minden mutation server-side owner/admin check;
   - organizer más eventjét URL-manipulációval sem érheti el;
   - admin override auditált.

2. Dashboard refactor:
   - előbb characterization tests;
   - Overview, Participants, Check-in, Messages, Analytics, Settings modul;
   - route/query selected event;
   - deep-link;
   - loading/error izoláció.

3. Participant ops:
   - bulk message;
   - bulk status csak safe transition;
   - check-in QR/invite-code terv, ha a jelenlegi invite_code ezt támogatja;
   - duplicate check-in prevention;
   - offline/poor-network graceful behavior;
   - participant private data minimization.

4. Messaging:
   - audience: going, waitlist, checked-in, selected;
   - subject/body validation;
   - rate limit;
   - audit;
   - notification delivery status;
   - későbbi email/push adapter, de provider nélkül ne ígérj deliveryt.
...
5. Analytics:
   ...
   - waitlist conversion;
   - repeat participants;
   - connection/circle outcomes aggregáltan, privacy-safe;
   - export CSV opcionális, explicit data authorization.

6. Organizer reliability:
   - cancellation rate;
   - event completion;
   - participant feedback aggregáltan;
   - ne legyen public punitive score első körben.

7. Event operations:
   - duplicate event/template;
   - cancel/reschedule with notification;
   - capacity change semantics;
   - audit log.

### Acceptance

Organizer nem fér hozzá más organizer PII-jához. Minden participant state transition validált state-machine-en megy. A korábbi dashboard flow-k regression testtel lockolva. A consumer Navbar/organizer mode viselkedés marad.
