# 08. Discovery, recommendation és matching
...
A feedet egyszerű eseménylistából magyarázható, privacy-safe relevance engine-né emelni, amely nem engagement-addikcióra, hanem valós részvétel valószínűségére és ismételt kapcsolódásra optimalizál.

### Candidate sources

- native events;
- imported Eventbrite/Ticketmaster/SeatGeek események;
- virtual hub alapján generált események;
- social circle következő activity;
- local venue/activity opportunity.

### Ranking feature-ek

Engedélyezett:
- explicit hobby/activity preference;
- coarse location/distance;
- chosen availability;
- event date;
- capacity;
- prior attended categories;
- connections/circles részvétele;
- organizer/event quality aggregate;
- novelty/diversity cap.

Tiltott vagy külön consent nélkül ne:
- egészség, politikai, vallási, szexuális vagy más érzékeny inferencia;
- „loneliness score”;
- dark pattern engagement scoring.

### Implementáció

1. `RecommendationCandidate` normalized domain.
2. deterministic baseline scorer;
3. reason codes:
   - „mert szereted a túrázást”,
   - „3 ismerősöd/circle-tagod érdeklődik” – csak privacy engedéllyel,
   - „közel van hozzád”;
4. diversity reranking;
5. source dedupe;
6. external/native canonical identity;
7. cold-start fallback;
8. no-result latent-demand CTA: „Értesíts / szervezzünk valamit”.

### Social matching

Ne user card swipe feed legyen. Matching csak activity-contextben:
- event company request;
- circle suggestion;
- hub demand aggregate;
- reconnection.
A matching score legyen server-side és reason-code-os.
...
- detail -> RSVP;
- RSVP -> attended;
- attended -> repeat encounter;
- repeat -> circle.
Ne csak CTR-rel optimalizálj.

### Acceptance

A ranking unit test fixture-ből determinisztikus. Blocked user sem közvetlenül, sem „3 ismerősöd” aggregátumban nem szivárog. Cold-start user explicit érdeklődéssel releváns feedet kap.
