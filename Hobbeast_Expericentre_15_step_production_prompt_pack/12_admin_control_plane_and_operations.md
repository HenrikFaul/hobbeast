# 12. Admin control plane és operations
...
Az admin felületet egységes production control plane-né tenni. Ne legyen minden adatbázis-mező szerkeszthető „god mode”; capability-based, auditált, biztonságos operációs eszköz kell.

### Admin információ-architektúra
...
- generated user marker;
- hub membership.
Fejlesztés:
- immutable IDs;
- public/private field separation;
- safe profile edit RPC;
- suspend/restore;
- delete request view;
- block/report moderation link;
- no arbitrary auth mutation browserből.

### Simulation users

- külön sandbox/simulation flag;
- mass-create limit;
- production notification kizárás;
- analytics kizárás;
- egygombos cleanup csak explicit destructive confirmationnel;
- real userrel soha ne keveredjen;
- hub test mode külön.

### System health

Dashboard:
- external sync last run;
- place/address jobs;
- provider health;
- notification queue;
- auto-event jobs;
- error rate;
- target project assertion;
- migration/version;
- build version;
- feature flags.
...
Minden privileged action:
actor, action, target_type/id, timestamp, result, correlation id, safe metadata.
Ne logolj secretet és szükségtelen PII-t.

### Bulk actions

Dry-run preview, count, target filter snapshot, idempotency, job progress, partial failure report, rollback ahol lehetséges.

### Acceptance

Admin route client-side guard mellett server-side authorizationnal is védett. Minden bulk/privileged művelet auditálható. Admin UI failure nem érinti consumer app availabilityt.
