# 05. Virtual Hubs 2.0 és latent community engine
...
A meglévő `virtual_hubs` + `virtual_hub_members` rendszert emeld production szintű latent-community engine-né. A jelenlegi modell hobby × city alapján admin-only, refresh funkcióval és auto-event integrációval működik. Ezt ne dobd ki; verziózd és tedd stabil, inkrementális, mérhető rendszerré.

### Feladatok

1. Audit:
   - migration, RLS, `refresh_virtual_hubs`, `virtual-hubs-admin`, AdminUsers hub UI, HubDetailModal, generate-hub-events;
   - jelenlegi teljes DELETE/rebuild viselkedés vs inkrementális update;
   - stale membership és concurrency.

2. Hub identity:
   - canonical hobby/activity ID, ne csak free-text;
   - normalized city/geo bucket;
   - optional radius/geohash strategy;
   - deterministic unique key;
   - generated user flag figyelembevétele: simulation ne torzítsa production metrics-et.

3. Incremental membership:
   - profile interest/location change esetén újraszámolás;
   - idempotens job;
   - full rebuild csak admin recovery;
   - member_count derived/consistent;
   - race-safe.

4. Hub qualification:
   - minimum valódi tag;
   - recent activity;
   - availability overlap későbbi AI eventhez;
   - existing event saturation;
   - organizer presence opcionális;
   - score magyarázható komponensekkel.

5. System Hub vs User-facing Circle:
   - virtual hub maradhat backend/system segmentation;
   - ne tedd automatikusan publikus chatroommá;
   - a hubból esemény és később circle születhet;
   - user számára max „X ember a környékeden érdeklődik” privacy-safe aggregate.

6. Admin:
   - hub list filter city/hobby/status;
   - real vs simulated member count;
   - qualification reason;
   - refresh job state;
   - stale/error state;
   - drilldown csak szükséges PII-val;
   - audit log.

7. Scheduler:
   - full refresh/reconciliation időzítése;
   - retry;
   - job lock;
...
Nincs destructive full rebuild minden kis profile change-nél. Nincs duplicate hub. Generated users nem számítanak bele éles „valódi kereslet” KPI-ba. A régi admin refresh funkció back-compat módon működik vagy biztonságos redirect/alias kap.
