# 14. Observability, performance, accessibility és quality engineering
...
A teljes platformot production-quality engineering szintre emelni: mérhető, diagnosztizálható, gyors, hozzáférhető és regressziókkal szemben védett legyen.
...
- job metrics;
- provider metrics;
- DB latency/error instrumentation adapter;
- release version;
- feature flag context;
- PII redaction.

### SLO-javaslatok dokumentálása
...
- notification delay;
- sync freshness.
Ne állíts production SLO teljesülést mérés nélkül.

### Performance

- bundle budget megtartása;
- route chunks;
- image optimization;
- lazy heavy maps/charts;
- query cache;
- list virtualization, ha valóban szükséges;
- DB index audit a gyakori event/profile/hub/notification querykra;
- N+1 eltávolítás;
- external provider server-side cache.
...
Critical E2E:
signup/login -> onboarding -> event discovery -> join -> organizer check-in -> completion -> reconnection -> circle;
plus admin smoke.

### Visual regression

Legalább landing, Events, EventDetail, Profile, Organizer, Admin kulcsnézet screenshot baseline. Ne pixel-lockolj dinamikus tartalmat stabil fixture nélkül.

### Chaos/failure

- provider outage;
- Supabase transient error;
- realtime disconnect;
- duplicate mutation;
- slow network;
- stale session;
...
Quality report machine-readable artifacttal. Performance budget CI-ben. Accessibility blocker count documented. Critical user journey E2E zöld production-like stagingben.
