# 04. Social graph: encounter, reconnection és social circle
...
Ez a Hobbeast stratégiai magja. Építsd meg a hiányzó relationship lifecycle-t úgy, hogy az event után ne érjen véget a user journey. A cél nem dating és nem követőszám, hanem közös valós aktivitásból kialakuló ismételt kapcsolódás.

### Új domain fogalmak

Tervezd meg és migráld biztonságosan:
- `event_encounters` vagy ekvivalens: kik voltak ténylegesen ugyanazon eseményen, kizárólag checked-in/completed részvételből;
- `reconnection_preferences`: privát jelzés „szívesen csinálnék vele újra valamit”;
- `connections`: kölcsönös, nem romantikus activity connection;
- `social_circles`: kisebb visszatérő csoport;
- `social_circle_members`;
- opcionális `circle_activity_history`.

A tényleges neveket a jelenlegi schema konvencióihoz igazítsd. Ne duplikáld a meglévő friend-request struktúrát, ha a migrationokban már van részleges barátmodell: előbb inventory.

### Event utáni flow

1. Event completion után csak tényleges résztvevők kapjanak post-event promptot.
2. „Milyen volt?” rövid, nem nyilvános feedback.
3. „Kikkel csinálnál újra valamit?” – csak azokból, akik valóban jelen voltak és akiket nem blokkolt.
4. A választás privát; a másik fél nem látja, ha nem kölcsönös.
5. Mutual esetén connection jön létre és notification.
6. A rendszer ajánlhat következő közös activityt, de nem küldhet spamet.
7. Több ismételt közös részvétel után circle suggestion.
8. Circle létrehozása explicit user consenttel történjen.

### Safety/privacy

- exact attendee list privacy;
- organizer nem láthat reconnection choice-ot;
- admin csak abuse/support célból, auditált privileged flow-val férhessen hozzá, ha egyáltalán szükséges;
- block azonnal megszünteti discovery/recommendation visibilityt;
- connection megszakítható;
- circle elhagyható;
- minors nincsenek specifikálva a jelenlegi requirementsben: ne találj ki korhatár-policyt; production launch előtt explicit policy blocker legyen.

### Algoritmikus jel

Készüljön egyszerű, magyarázható `connection_strength` proxy belső használatra:
- közös completed események száma;
- időbeli frissesség;
- mutual reconnection;
- közös érdeklődések;
- circle membership.
Ne használj érzékeny inferenciát, ne legyen „compatibility personality score”.

### UI

- Post-event check-in/feedback flow mobilon;
- Connections nézet;
- Circles nézet;
- circle detail: tagok, közös érdeklődések, következő program;
- „Szervezzünk újra valamit” CTA;
- empty state: ne legyen szégyenítő.

### Acceptance

Két user csak közös, completed esemény után választhatja egymást reconnectionre. Egyoldalú választás titkos marad. Mutualból idempotensen egy connection jön létre. Block minden social surface-t kizár. Circle suggestion repeat evidence nélkül nem készül.
