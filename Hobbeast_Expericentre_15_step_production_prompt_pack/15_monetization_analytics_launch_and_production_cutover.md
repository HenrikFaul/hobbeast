# 15. Monetization, analytics, launch és production cutover
...
Az utolsó lépésben ne csak „deployold”. Készíts mérhető, rollbackolható production launchot, amely megőrzi a Hobbeast core küldetését. A monetizáció ne ösztönözze a screen time-ot a valódi kapcsolódással szemben.
...
- interest_selected;
- event_impression/detail/join;
- waitlist;
- checked_in;
- completed;
- post_event_feedback;
- reconnection_sent/mutual;
- circle_created/joined;
- organizer_event_created/completed;
- hub_qualified;
- auto_event_proposed/published.
PII ne analytics payloadban.

### North-star dashboard

Primary outcome:
**Meaningful Real-World Connections Created** – csak világos, proxy definícióval, például mutual reconnection + repeat completed encounter, nem pszichológiai diagnózis.

Supporting:
- attended events;
- repeat encounter rate;
- active circles;
- organizer completion;
- RSVP->attendance;
- hub->event conversion;
- external event social conversion.

### Monetization hypothesis – implementáció csak feature-flaggel

Vizsgáld:
- organizer Pro tooling;
- promoted local experiences, világosan jelölt;
- venue/business partner lead;
- premium club/circle organizer features;
- B2B community benefit;
- transaction fee csak ticketing esetén.
Ne paywallozd az alapvető block/report/privacy funkciókat.
Ne implementálj fizetési providert kulcs/jogi/tax döntés nélkül. Hozz létre pricing/entitlement architecture-t providerfüggetlenül.

### Feature flags

Minden nagy új social feature:
- connections;
- circles;
- hub2;
- AI proposals;
...
- moderation;
- analytics.
Rollout cohort és kill switch.

### Data migration/cutover

- schema backup/restore plan;
- migration ordering;
- compatibility window;
- backfill progress;
- zero/low downtime;
- rollback limitations dokumentálva;
- external sync pause/resume plan.

### Production checklist

- secrets rotated/operator verified;
- Supabase target verified;
- RLS audit;
- SECURITY DEFINER;
- auth redirect domains;
- OAuth production domain;
- provider quotas;
- cron;
- storage;
- email/push only if configured;
...
internal -> generated/simulation -> small real cohort -> city beta -> general availability.
Minden szakaszhoz exit criteria és rollback.

### Go/no-go

Készíts `docs/PRODUCTION_GO_LIVE_CHECKLIST.md` és `docs/GO_NO_GO_REPORT.md`.
Csak akkor írj GO-t, ha minden P0/P1 blocker evidence alapján zöld. Ha provider secret rotation, legal/policy vagy production OAuth nincs ellenőrizve, akkor CONDITIONAL/NO-GO, nem „valószínűleg jó”.

### Végső elv

A launch után is a terméknek azt kell optimalizálnia, hogy a user a telefonból a való életbe jusson. A siker nem az, hogy mennyi ideig marad az appban, hanem hogy az app segítségével létrejött-e valós, ismétlődő, biztonságos közös aktivitás.
