...
// supabase/functions/shared/ticketmaster.ts
var BASE_URL = "https://app.ticketmaster.com/discovery/v2/events.json";
function envApiKey() {
...
function normalizeTicketmasterEvent(event) {
  ...
  const localStart = event?.dates?.start?.localDate && event?.dates?.start?.localTime ? `${event.dates.start.localDate}T${event.dates.start.localTime}` : event?.dates?.start?.dateTime ?? null;
...
async function fetchTicketmasterEvents(params) {
  ...
  const data = await fetchJson(url.toString(), { method: "GET" }, "Ticketmaster fetch failed", { timeoutMs: 12e3, retries: 2 });
...
serve(async (req) => {
  ...
  try {
    ...
    if (!message.includes("PROVIDER_DISABLED") && !message.includes("PROVIDER_CIRCUIT_OPEN")) {
      await failExternalProviderRun(admin, runId, "ticketmaster", error).catch(() => void 0);
    }
