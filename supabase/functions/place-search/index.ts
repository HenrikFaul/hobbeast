...
const PLACE_SEARCH_RUNTIME_VERSION = 'v1.7.6-stable-db-autocomplete'
...
function errorJson(error: unknown, fallbackStatus = 500) {
  ...
  return json({ error: message, details, runtime_version: PLACE_SEARCH_RUNTIME_VERSION }, status)
}
...
function rowHasUsefulVenueSignal(row: Record<string, any>) {
...
function scoreDbAutocompleteRow(row: ProviderPlace, query: string, strategy: string, center?: Coordinates | null) {
...
async function autocompleteGeodataPlaces(tableConfig: DbSearchTableConfig, params: SearchBody): Promise<{ results: ProviderPlace[]; debug: Record<string, unknown> }> {
  ...
  const fetchedRows = await restFetchJson<Record<string, any>[]>(restUrl.toString(), { headers: geodataHeaders(key) })
  const rawRows = Array.isArray(fetchedRows) ? fetchedRows : []
  let categoryPrefetchRows: Record<string, any>[] = []
  if (categoryTerms.length > 0 && available.has('categories') && tableConfig.table !== 'public.local_pois') {
    ...
    if (categoryKeys.length > 0) {
      ...
      categoryPrefetchRows = await restFetchJson<Record<string, any>[]>(categoryUrl.toString(), { headers: geodataHeaders(key) }).catch(() => [])
    }
  }
...
function rowMatchesQueryFuzzy(row: Record<string, any>, query?: string) {
