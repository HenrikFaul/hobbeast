-- Expose country_code on the admin sources table so the operator can see and
-- filter by country now that the collector covers CZ/SK/PL/AT/SI as well as HU.
-- Additive: one extra key in each destination object; the row set, ordering and
-- every existing key are unchanged.
